...
class GnosisWallet {
  private ethereumApi: Eth;

  constructor(ethereumApi: Eth) {
    this.ethereumApi = ethereumApi;
  }

  public getExecutionResult = (address: string, fromBlock?: string | number): Promise<ExecutionResult> => {
    const contract = new this.ethereumApi.Contract(gnosisSafeAbi, address);
    const block = fromBlock || this.ethereumApi.defaultBlock.toString();
    return new Promise((resolve, reject): void => {
      contract.once(
        ...
        (e: Error, ev: EventData): void => {
          if (e) reject(e);
          const { returnValues, transactionHash } = ev;
          if (!returnValues) reject(new Error(`failed to parse events`));
          const { txHash, payment } = returnValues;
          if (!txHash) reject(new Error(`invalid transaction hash '${txHash}'`));
          resolve({
            success: true,
