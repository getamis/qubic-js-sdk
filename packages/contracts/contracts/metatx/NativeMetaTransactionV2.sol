...
import './INativeMetaTransactionV2.sol';
...
//     }
//
contract NativeMetaTransactionV2 is NativeMetaTransaction, INativeMetaTransactionV2 {
  bytes32 private constant META_TRANSACTION_TYPEHASH =
    keccak256(
      bytes('MetaTransaction(address relayer,address authorizer,bytes32 nonce,bytes callData,uint256 deadline)')
    );
   ...
   * @param relayer       The relayer's address
   * @param authorizer    Authorizer's address
   * @param nonce         Nonce of the authorization
  ...
  function _requireValidAuthorization(
    ...
    require(block.timestamp <= deadline, 'NativeMetaTransaction: authorization is expired');
  ...
  function supportsInterface(bytes4 interfaceId) external view virtual returns (bool) {
    return interfaceId == type(INativeMetaTransactionV2).interfaceId;
  }
}
